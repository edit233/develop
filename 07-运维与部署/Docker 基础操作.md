...
## 相关笔记
- [[Nginx 部署实战]] —— Docker 内 Nginx
- [[Linux 运维操作]] —— 服务器基础
- [[Python 项目构建整体模板]] —— Python 应用容器化
- [[Java 项目构建整体模板]] —— Java 应用容器化

## 参考
- [Docker 官方文档](https://docs.docker.com/)
- [Dockerfile 指令参考](https://docs.docker.com/engine/reference/builder/)
- [Docker Compose 文档](https://docs.docker.com/compose/)
## 实战案例：Spring Boot + Nginx 多模块项目 Docker 化

### 多阶段构建（Multi-stage Build）

后端是 Maven 多模块项目，需要先编译再运行。多阶段构建用一个镜像编译，另一个镜像运行，最终镜像只包含运行时文件，体积小：
...
关键点：
- AS build 命名阶段，COPY --from=build 引用前一阶段产物
- 先 COPY pom.xml 再 RUN 再 COPY 源码，利用 Docker 层缓存避免每次重新下载依赖
- 最终镜像基于 JRE（不含 JDK/Maven），体积大幅缩小

### Spring Boot 环境变量覆盖

Docker 化后数据库密码不应硬编码在 application.yaml 中，通过环境变量注入：

```yaml
# application.yaml 中保留默认值
spring:
  datasource:
...
Spring Boot 3.x 自动支持环境变量覆盖（属性名大写+下划线）：

| application.yaml 属性 | 环境变量 |
|---|---|
...
```yaml
environment:
  - SPRING_DATASOURCE_URL=jdbc:mysql://${MYSQL_HOST:-localhost}:3306/${MYSQL_DB:-qk}?useUnicode=true&characterEncoding=utf-8&serverTimezone=Asia/Shanghai
  - SPRING_DATASOURCE_USERNAME=${MYSQL_USER:-root}
  - SPRING_DATASOURCE_PASSWORD=${MYSQL_PASSWORD:-123456}
```

### Docker Compose 完整实战（后端+前端+MySQL）
...
services:
  ...
  mysql:
    image: mysql:8.0
    container_name: qk-mysql
    restart: always
    environment:
      MYSQL_ROOT_PASSWORD: ${MYSQL_PASSWORD:-123456}
      MYSQL_DATABASE: ${MYSQL_DB:-qk}
    ports:
      - "3306:3306"
    volumes:
      - mysql_data:/var/lib/mysql
...
- ./sql:/docker-entrypoint-initdb.d MySQL 首次启动自动执行该目录下的 SQL 脚本
- mysql_data 命名卷保证数据库数据持久化，容器重建不丢数据
- .env 文件管理敏感配置，通过 ${VAR:-默认值} 语法注入

### 国内 Docker Hub 镜像加速

国内服务器拉取 Docker Hub 镜像常超时，配置国内镜像源：
...
> 注意：镜像源可能失效，失效时需要更换其他源。常见备选：https://docker.rainbond.cc、https://registry.docker-cn.com。

### Docker 用户权限问题

非 root 用户执行 docker 命令会报 permission denied while trying to connect to the docker API。解决方案：

```bash
# 方案一：加入 docker 组（推荐，一劳永逸）
