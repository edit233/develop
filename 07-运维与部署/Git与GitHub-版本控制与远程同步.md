...
| 类别 | 忽略内容 | 原因 |
|---|---|---|
| IDE | .idea/、*.iml、.vscode/ | 个人 IDE 配置 |
| 构建产物 | 	arget/、dist/、
ode_modules/ | 可重新生成 |
| 运行时 | .env、*.log、logs/ | 含敏感信息或临时数据 |
| OS | .DS_Store、Thumbs.db | 系统生成的无用文件 |
| Docker | .env（实际密码） | 仓库中只保留 .env.example 作为模板 |

注意：.gitignore 之前已经被 Git 跟踪的文件不会被忽略，需要先 git rm --cached 移除。

## 补充：GitHub Actions 自动部署

通过 SSH 实现推送代码后服务器自动拉取并重建容器。

工作流文件 .github/workflows/deploy.yml 核心逻辑：
1. 触发条件：push 到 main 分支或手动触发
2. 通过 ppleboy/ssh-action SSH 连接服务器
3. 执行 git pull → docker compose down → build → up -d

需要在 GitHub 仓库 Settings → Secrets and variables → Actions 添加三个 Secret：

| Secret 名称 | 说明 |
|---|---|
| SERVER_HOST | 服务器 IP 或域名 |
| SERVER_USER | SSH 用户名 |
| SSH_PRIVATE_KEY | 服务器的 SSH 私钥完整内容 |

服务器上需要将部署专用公钥追加到 ~/.ssh/authorized_keys。服务器上的仓库目录（如 /opt/qk-management）首次克隆后，后续由 Actions 自动 pull 更新。
